...
```{r, load sediment flux data -- MISSING IN ORIGINAL}
# BUG FIX: the original script used `sed` and `veg` below without ever
# creating them -- this chunk was missing entirely, so the join two chunks
# down would fail with "object 'sed' not found."
#
# TODO: point this at your actual sediment flux CSV (the file with
# FluxAvgRep, Site, Season, Distance, Transect, Year, Month columns).
sed <- read_xlsx("C:\\Users\\savan\\OneDrive - San Francisco State University\\ProjectData\\GitHub\\DepositionSFB\\data\\usgs_thesis.xlsx")  
# TODO: confirm actual filename

# `veg` (singular) is referenced later (sed_DistPlant chunk) as if it's a
# combined thesis + usgs vegetation table. Building that here from the two
# data frames already loaded above. TODO: confirm this is the intended
# combination -- thesis_veg and usgs_veg have different Site/Plot naming
# conventions (Plot_ID vs Plot.ID) that were reconciled into Tran/Dist
# above, but double check Site values match between the two before trusting
# any Site-level comparison across thesis vs usgs data.
veg <- bind_rows(
...
# NOTE (not fixed here, flagging for the modeling-design conversation):
# FluxAvgRep is a continuous sediment deposition rate, not count data, so
# Poisson (mean = variance, integer support) is likely the wrong family --
# your Methodological Plan calls for Gamma or log-normal instead. Leaving
# this as-is since it's the historical class analysis, not touching the
# stats until you've had a chance to chew on the modeling design.
...
# NOTE: (1|Site) below has the same singular-fit risk flagged in your
# RQ2 modeling plan doc -- if sed_SAPA only has 2 Site levels, lme4/glmmTMB
# won't be able to estimate that variance component reliably. Worth the
# same fix (Site as fixed effect) if this model gets carried forward.
