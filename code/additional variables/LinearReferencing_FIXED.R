...
# BUG FIX (quick pass): plot_transects(), plot_faceted_location(), and
# plot_overlay_location() were each defined 2-4 times across this script
# (standard scale, 50m, first 5 meters, 5m around pin) using the SAME
# function name every time. Run top to bottom in one session this doesn't
# break anything, but if this ever gets sourced by another script, or if
# someone runs sections out of order, only the LAST definition survives and
# earlier calls silently use the wrong plotting logic. Renamed each to be
# scale-specific (e.g. plot_transects_standard, plot_transects_50m,
# plot_transects_first5m, plot_transects_5m_around_pin) so they coexist
# safely. No logic inside any function was changed.
#
# STILL UNCHANGED: hardcoded Windows path in setwd() below -- update to
# your current machine/repo location before running.
...
plot_transects_standard <- function(file, location_name) {
  df <- read_csv(file)
...
for (loc in names(file_paths)) {
  plot_transects_standard(file_paths[[loc]], loc)
}
...
plot_faceted_location_standard <- function(file, location_name) {
  df <- read_csv(file)
...
if (run_faceted_plots) {
  for (loc in names(file_paths)) {
    plot_faceted_location_standard(file_paths[[loc]], loc)
  }
}
...
plot_overlay_location_standard <- function(file, location_name) {
  df <- read_csv(file)
...
if (run_overlay_plots) {
  for (loc in names(file_paths)) {
    plot_overlay_location_standard(file_paths[[loc]], loc)
  }
}
...
# Define the plot function
plot_transects_50m <- function(file, location_name) {
...
# Run it
for (loc in names(file_paths)) {
  plot_transects_50m(file_paths[[loc]], loc)
}

# Faceted plot for all transects at a location
plot_faceted_location_50m <- function(file, location_name) {
  df <- read_csv(file) %>% filter(Distance_AlongTransect_m <= 50)
...
if (run_faceted_plots) {
  for (loc in names(file_paths)) {
    plot_faceted_location_50m(file_paths[[loc]], loc)
  }
}

####OVERLAID PLOTS####

plot_overlay_location_50m <- function(file, location_name) {
  df <- read_csv(file) %>% filter(Distance_AlongTransect_m <= 50)
...
if (run_overlay_plots) {
  for (loc in names(file_paths)) {
    plot_overlay_location_50m(file_paths[[loc]], loc)
  }
}
...
plot_transects_first5m <- function(file, location_name) {
  df <- read_csv(file) %>%
...
for (loc in names(file_paths)) {
  plot_transects_first5m(file_paths[[loc]], loc)
}
...
# Define the plotting function
plot_transects_5m_around_pin <- function(file, location_name) {
  df <- read_csv(file) %>%
    filter(Distance_AlongTransect_m <= 50)
...
# Run the function on all locations
for (loc in names(file_paths)) {
  plot_transects_5m_around_pin(file_paths[[loc]], loc)
}
